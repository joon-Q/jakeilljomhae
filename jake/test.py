...
parser.add_argument("--dataset_dir", type=str, default='/database/nam/Kalantari',
...
parser.add_argument('--pretrained_model', type=str, default='/database/wheatley97/ckpt/val_latest_checkpoint.pth') 
...
def main():
    ...
    for idx, img_dataset in enumerate(datasets):
        ...
        pred_img_mu2 = cv2.cvtColor(pred_img_mu, cv2.COLOR_RGB2GRAY)
        cv2.imwrite('./test_pred.png',pred_img_mu2)

        label_mu = np.clip(label_mu * 255.0, 0., 255.).transpose(1, 2, 0)
        label_mu2 = cv2.cvtColor(label_mu, cv2.COLOR_RGB2GRAY)
        cv2.imwrite('./test_label.png',label_mu2)

        mask = abs(pred_img_mu2 - label_mu2)
        mask = np.where(mask>10, 255, mask)
        ...
        if args.save_results:
            ...
            if not osp.exists('./tone_mapped_results/'):
                os.makedirs('./tone_mapped_results/')
            cv2.imwrite('./tone_mapped_results/label_' + str(idx) + '.png', label_mu)
            cv2.imwrite('./tone_mapped_results/pred_' + str(idx) + '.png', pred_img_mu)
