...
from models.loss import L1MuLoss, JointReconPerceptualLoss, JointReconPerceptualLoss2
...
def get_args():
    parser = argparse.ArgumentParser(description='HDR-Transformer2',
...
def train(args, model, device, train_loader, optimizer, epoch, criterion, criterion2, scheduler):
    model.eval()
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        batch_ldr0, batch_ldr1, batch_ldr2, batch_ldr0_fake, batch_ldr2_fake = batch_data['input0'].to(device), batch_data['input1'].to(device), batch_data['input2'].to(device), \
        batch_data['input0_fake'].to(device), batch_data['input2_fake'].to(device)
        label = batch_data['label'].to(device)
        
        pred, fake_lr, fake_hr = model(batch_ldr0, batch_ldr1, batch_ldr2)
           
        loss = criterion(pred, label) + criterion2(fake_lr, batch_ldr0_fake[:,3:6, :, :]) + criterion2(fake_hr, batch_ldr2_fake[:,3:6, :, :])
    ...
    scheduler.step()
    wandb.log({"Train_loss": loss_total/len(train_loader)})
    num = 0
...
def validation(args, model, device, val_loader, optimizer, epoch, criterion, cur_psnr):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(val_loader):
            ...
            label = batch_data['label'].to(device)
            pred, _, _ = model(batch_ldr0, batch_ldr1, batch_ldr2)
...
def test_single_img(model, img_dataset, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, total=len(dataloader)):
            ...
            output, out_lr, out_hr = model(batch_ldr0, batch_ldr1, batch_ldr2)
            img_dataset.update_result(torch.squeeze(output.detach().cpu()).numpy().astype(np.float32))
            img_dataset.update_result_lr(torch.squeeze(out_lr.detach().cpu()).numpy().astype(np.float32))
            img_dataset.update_result_hr(torch.squeeze(out_hr.detach().cpu()).numpy().astype(np.float32))
    pred, label = img_dataset.rebuild_result()
    pred_lr, _ = img_dataset.rebuild_result_lr()
    pred_hr, _ = img_dataset.rebuild_result_hr()

    return pred, label, pred_lr, pred_hr
...
def test(args, model, device, optimizer, epoch, cur_psnr, **kwargs):
    ...
    for idx, img_dataset in enumerate(test_datasets):
        pred_img, label, _, _ = test_single_img(model, img_dataset, device)
        scene_psnr_l = psnr(label, pred_img)

        label_mu = range_compressor(label)
        pred_img_mu = range_compressor(pred_img)

        scene_psnr_mu = psnr(label_mu, pred_img_mu)
    ...
    wandb.log({"psnr_l": psnr_l.avg, "psnr_mu": psnr_mu.avg, "ssim_l.avg": ssim_l.avg, "ssim_mu.avg": ssim_mu.avg})
    # save_model
    save_dict = {
        'epoch': epoch + 1,
        'state_dict': model.state_dict(),
...
def main():
    ...
    # model architectures
    model = HDRTransformer(embed_dim=60, depths=[6, 6, 6], num_heads=[6, 6, 6], mlp_ratio=4, in_chans=6)
    ...
    criterion2 = JointReconPerceptualLoss2().to(device)
    # optimizer
    ...
    for epoch in range(args.epochs):
        ...
        train(args, model, device, train_loader, optimizer, epoch, criterion, criterion2, scheduler)        
