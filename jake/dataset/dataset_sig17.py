...
class SIG17_Training_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ldr_images_0_fake = np.power(np.clip(label * expoTimes[0], 0, 1), 1./2.2)
        ldr_images_2_fake = np.power(np.clip(label * expoTimes[2], 0, 1), 1./2.2)

        pre_img0_fake = ldr_to_hdr(ldr_images_0_fake, expoTimes[0], 2.2)
        pre_img2_fake = ldr_to_hdr(ldr_images_2_fake, expoTimes[2], 2.2)

        pre_img_0_fake = np.concatenate((pre_img0_fake, ldr_images_0_fake), 2)
        pre_img_2_fake = np.concatenate((pre_img2_fake, ldr_images_2_fake), 2)
        ...
        img_0_fake = pre_img_0_fake.astype(np.float32).transpose(2, 0, 1)
        img_2_fake = pre_img_2_fake.astype(np.float32).transpose(2, 0, 1)

        img0 = torch.from_numpy(img0)
        img1 = torch.from_numpy(img1)
        img2 = torch.from_numpy(img2)
        label = torch.from_numpy(label)
        img_0_fake = torch.from_numpy(img_0_fake)
        img_2_fake = torch.from_numpy(img_2_fake)
        ...
        sample = {
            ...
            'input2': img2, 
            'label': label,
            'input0_fake' : img_0_fake,
            'input2_fake' : img_2_fake
            }
...
class Img_Dataset(Dataset):
    ...
    def rebuild_result_lr(self):
        ...
        for x in range(n_w):
            for y in range(n_h):
                pred[:, y*self.patch_size:(y+1)*self.patch_size, x*self.patch_size:(x+1)*self.patch_size] = self.result_lr[x*n_h+y]
        return pred[:, :h, :w], self.label.transpose(2, 0, 1)
    
    def rebuild_result_hr(self):
        h, w, c = self.label.shape
        n_h = h // self.patch_size + 1
        n_w = w // self.patch_size + 1
        tmp_h = n_h * self.patch_size
        ...
        for x in range(n_w):
            for y in range(n_h):
                pred[:, y*self.patch_size:(y+1)*self.patch_size, x*self.patch_size:(x+1)*self.patch_size] = self.result_hr[x*n_h+y]
        return pred[:, :h, :w], self.label.transpose(2, 0, 1)
    ...
    def update_result_lr(self, tensor):
        self.result_lr.append(tensor)

    def update_result_hr(self, tensor):
        self.result_hr.append(tensor)
